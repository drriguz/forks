...
public class Query {
    ...
    public static Query build(String select, Condition... args) {
        ...
        for (Condition arg : args) {
            if (arg.getParam() == null || (arg.getParam() instanceof String && Strings
                .isNullOrEmpty((String) arg.getParam()))) {
                continue;
            }
            if (arg.queryType == QUERY.ORDERBY) {
                ...
                builder.append(
                    MessageFormat.format(" AND `{0}` {1} ?", arg.getFieldName(), arg.getQueryType().toString()));
