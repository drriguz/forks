...
public final class Beans {
    ...
    public static <T> T bind(Class<T> beanClass, String prefix, Map<String, String[]> parameters, String[] fields,
        boolean errorIfNotExist, boolean errorIfConvertFailed) {
        ...
        for (String field : fields) {
            ...
            if (!methodDict.containsKey(setterName)) {
                throw new IllegalArgumentException("Specified attribute setter not found:" + field);
            }

            Method setter = methodDict.get(setterName);
            Class<?>[] arguments = setter.getParameterTypes();
            if (arguments == null || arguments.length != 1) {
                throw new IllegalArgumentException("Specified attribute setter not valid:" + field);
            }

            String paramName = field;
            if (!Strings.isNullOrEmpty(prefix)) {
                paramName = prefix + "." + field;
            }

            if (!parameters.containsKey(field) && errorIfNotExist) {
                throw new RuntimeException("No value found for field[" + field + "] the name is:" + paramName);
            }
            try {
